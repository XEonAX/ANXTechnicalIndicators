...
class Renko extends Indicator {
  ...
  constructor(input: RenkoInput) {
    ...
    if (brickSize === 0) {
      console.error(
        "Not enough data to calculate brickSize for renko when using ATR",
      );
      return;
    }
    let lastOpen = 0;
    ...
    this.generator = (function* () {
      let candleData: CandleData = yield;
      while (true) {
        ...
        if (
          absoluteMovementFromClose >= brickSize &&
          absoluteMovementFromOpen >= brickSize
        ) {
          let reference =
            absoluteMovementFromClose > absoluteMovementFromOpen
              ? lastOpen
              : lastClose;
          let calculated = <any>{
            open: reference,
