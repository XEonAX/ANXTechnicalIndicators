...
export class IchimokuCloud extends Indicator {
  ...
  constructor(input: IchimokuCloudInput) {
    ...
    var currentConversionData = new LinkedList(
      params.conversionPeriod * 2,
      true,
      true,
      false,
    );
    var currentBaseData = new LinkedList(
      params.basePeriod * 2,
      true,
      true,
      false,
    );
    var currenSpanData = new LinkedList(
      params.spanPeriod * 2,
      true,
      true,
      false,
    ...
    this.generator = (function* () {
      ...
      while (true) {
        ...
        if (periodCounter < period) {
          ...
          let conversionLine =
            (currentConversionData.periodHigh +
              currentConversionData.periodLow) /
            2;

          // Kijun-sen (Base Line): (26-period high + 26-period low)/2))
          let baseLine =
            (currentBaseData.periodHigh + currentBaseData.periodLow) / 2;
          ...
          let spanB =
            (currenSpanData.periodHigh + currenSpanData.periodLow) / 2;
